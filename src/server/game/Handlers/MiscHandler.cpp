...
void WorldSession::HandleGossipSelectOptionOpcode(WorldPacket& recvData)
{
	...
	if (!code.empty())
	{
		...
		else if (item)
		{
			...
			sEluna->HandleGossipSelectOption(GetPlayer(), item, sender, action, code);
		}
		else
		{
			uint32 sender = _player->PlayerTalkClass->GetGossipOptionSender(gossipListId);
			...
			sEluna->HandleGossipSelectOption(GetPlayer(), menuId, sender, action, code);
		}
	}
	else
	{
...
void WorldSession::HandleWhoOpcode(WorldPacket& recvData)
{
	...
	if (sWorld->getBoolConfig(CONFIG_FAKE_WHO_LIST) && displaycount < 49)  //fake_char
	{
		// Fake players on WHO LIST                                    0,         1,        2,       3,     4,       5 ,       6 
		QueryResult result = WorldDatabase.Query("SELECT name, gname, level, class, race,  zone, gender FROM fake_char;");
		if (result)
		{
			do
			{
				...
				std::string gname = fields[1].GetString();   // guild name
				uint32 lvl = fields[2].GetUInt32();                // player level 
				uint32 class_ = fields[3].GetUInt32();            // player class 
				uint32 race = fields[4].GetUInt32();            // player race 
