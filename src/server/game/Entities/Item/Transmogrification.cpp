...
const char* Transmogrification::GetSlotName(uint8 slot, WorldSession* /*session*/) const
{
	...
	switch (slot)
	{
	case EQUIPMENT_SLOT_HEAD: return  " 头盔幻化";// session->GetTrinityString(LANG_SLOT_NAME_HEAD);
	case EQUIPMENT_SLOT_SHOULDERS: return  " 肩膀幻化";// session->GetTrinityString(LANG_SLOT_NAME_SHOULDERS);
	case EQUIPMENT_SLOT_BODY: return  " 衬衣幻化";// session->GetTrinityString(LANG_SLOT_NAME_BODY);
	case EQUIPMENT_SLOT_CHEST: return  " 胸甲幻化";// session->GetTrinityString(LANG_SLOT_NAME_CHEST);
	case EQUIPMENT_SLOT_WAIST: return  " 腰带幻化";// session->GetTrinityString(LANG_SLOT_NAME_WAIST);
	case EQUIPMENT_SLOT_LEGS: return  " 腿甲幻化";// session->GetTrinityString(LANG_SLOT_NAME_LEGS);
	case EQUIPMENT_SLOT_FEET: return  " 战靴幻化";// session->GetTrinityString(LANG_SLOT_NAME_FEET);
	case EQUIPMENT_SLOT_WRISTS: return  " 手腕幻化";// session->GetTrinityString(LANG_SLOT_NAME_WRISTS);
	case EQUIPMENT_SLOT_HANDS: return  " 手套幻化";// session->GetTrinityString(LANG_SLOT_NAME_HANDS);
	case EQUIPMENT_SLOT_BACK: return  " 披风幻化";// session->GetTrinityString(LANG_SLOT_NAME_BACK);
	case EQUIPMENT_SLOT_MAINHAND: return  " 主手幻化";// session->GetTrinityString(LANG_SLOT_NAME_MAINHAND);
	case EQUIPMENT_SLOT_OFFHAND: return  " 副手幻化";// session->GetTrinityString(LANG_SLOT_NAME_OFFHAND);
	case EQUIPMENT_SLOT_RANGED: return  " 远程幻化";// session->GetTrinityString(LANG_SLOT_NAME_RANGED);
	case EQUIPMENT_SLOT_TABARD: return  " 战袍幻化";// session->GetTrinityString(LANG_SLOT_NAME_TABARD);
	default: return NULL;
	}
}
