...
public:
	...
	bool OnGossipHello(Player* player, Creature* creature) override
	{
		...
		player->ADD_GOSSIP_ITEM_EXTENDED(1, "|TInterface\\ICONS\\Spell_Holy_HopeAndGrace:30:30:-18:0|t |cFF330066 变更角色姓名 " + sMachinistCore->GetItemLink(changePlayerInfoItemID, session) + " x " + sMachinistCore->ConvertNumberToString(changePlayerInfoItemCount), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1, "你确定需要变更姓名?", 0, false);
		player->ADD_GOSSIP_ITEM_EXTENDED(1, "|TInterface\\ICONS\\Spell_Holy_HopeAndGrace:30:30:-18:0|t |cFF330066 变更角色外观 " + sMachinistCore->GetItemLink(changePlayerInfoItemID, session) + " x " + sMachinistCore->ConvertNumberToString(changePlayerInfoItemCount), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2, "你确定需要变更外观?", 0, false);
		player->ADD_GOSSIP_ITEM_EXTENDED(1, "|TInterface\\ICONS\\Spell_Holy_HopeAndGrace:30:30:-18:0|t |cFF330066 变更角色阵营 " + sMachinistCore->GetItemLink(changePlayerInfoItemID, session) + " x " + sMachinistCore->ConvertNumberToString(changePlayerInfoItemCount), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3, "你确定需要变更阵营?", 0, false);
		player->ADD_GOSSIP_ITEM_EXTENDED(1, "|TInterface\\ICONS\\Spell_Holy_HopeAndGrace:30:30:-18:0|t |cFF330066 变更角色种族 " + sMachinistCore->GetItemLink(changePlayerInfoItemID, session) + " x " + sMachinistCore->ConvertNumberToString(changePlayerInfoItemCount), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 4, "你确定需要变更种族?", 0, false);
		player->ADD_GOSSIP_ITEM_EXTENDED(1, "|TInterface\\ICONS\\Spell_Holy_HopeAndGrace:30:30:-18:0|t |cFF330066 变更角色职业 " + sMachinistCore->GetItemLink(changePlayerInfoItemID, session) + " x " + sMachinistCore->ConvertNumberToString(changePlayerClassItemCount), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 5, "你确定需要变更职业?\n\n 只能变更本种族可以选择的职业! \n\n 并且脱掉所有装备 \n\n解散所有宠物 \n\n请阅读说明后谨慎选择! ", 0, false);
	...
	bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action) override
	{
		...
		case GOSSIP_ACTION_INFO_DEF + 5:
			...
			if (pet)
			{
				player->GetSession()->SendNotification("变更职业需要解散所有宠物, 请确认您已经解散所有宠物!");
				ChatHandler(player->GetSession()).SendSysMessage("变更职业需要解散所有宠物, 请确认您已经解散所有宠物!");
				player->CLOSE_GOSSIP_MENU();
				return true;
			}
